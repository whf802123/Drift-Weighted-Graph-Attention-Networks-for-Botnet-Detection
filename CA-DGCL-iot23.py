...
if 'label' not in df.columns:
    raise KeyError(
        "Column 'label' was not found. Please confirm that the CSV label column is named lowercase label."
    )
...
print(f"Minority classes kept intact: {sorted(MANUAL_MINORITY_LABELS)}")
print(f"Sampling {SAMPLE_FRAC * 100:.1f}% from all other classes.")


def keep_or_sample(group):
    lab = str(group.name)
...
if too_small:
    print(
        f"Classes with fewer than two samples are dropped: "
...
if feat_df.shape[1] == 0:
    raise RuntimeError(
        "No feature columns could be constructed."
    )
...
if NUM_CLASSES != len(np.unique(labels)):
    raise RuntimeError(
        "The training split does not contain every class."
    )
...
def propagated_features(x_np):
    a_hat = normalized_adjacency_numpy(
        x_np
    )
...
def fill_chain_tensor(
    ...
    for c_idx, chain in enumerate(
        chains
    ):
        observed = []

        for t, node_id in enumerate(
            chain
        ):
            if (
                node_id is not None
...
def hooi_factors(
    ...
    u = top_left_singular_vectors(
        unfold(x, 0),
        r1,
    )

    v = top_left_singular_vectors(
        unfold(x, 1),
        r2,
    )

    w = top_left_singular_vectors(
        unfold(x, 2),
        r3,
    )
...
def historical_node_factors(
    snapshot_features,
):
    if len(snapshot_features) < 2:
        return None

    chains = build_node_chains(
        snapshot_features
    )

    tensor = fill_chain_tensor(
        snapshot_features,
        chains,
    )
...
def build_extended_train_graph(
    x_train_np,
    generated_np,
):
    n_orig = len(
...
history_snapshots = deque(
    maxlen=MAX_HISTORY_SNAPSHOTS
)
