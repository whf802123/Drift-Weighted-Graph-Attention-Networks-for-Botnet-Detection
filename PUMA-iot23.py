...
if too_small:
    print(f"Classes with fewer than two samples are dropped: {sorted(list(too_small))}")
...
labels = np.array([label_to_id[x] for x in label_text], dtype=np.int64)
...
if NUM_CLASSES != len(np.unique(labels)):
    raise RuntimeError("The training split does not contain all IoT-23 classes.")
...
print("IoT-23 label mapping:", label_to_id)
print(
